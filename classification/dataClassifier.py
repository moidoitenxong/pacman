...
def enhancedFeatureExtractorDigit(datum):
    ...
    Add new binay features
    Dem so cac vung trang
    ##
    """
    ...
    count = 0
    visited = [ [False for j in range(DIGIT_DATUM_WIDTH)] for i in range(DIGIT_DATUM_HEIGHT) ]
    
    for i in range(DIGIT_DATUM_HEIGHT):
        for j in range(DIGIT_DATUM_WIDTH):
            #check if empty and not visited
            if pixelMatrix[i][j] == 0 and visited[i][j] == False:
                visited = DFS(pixelMatrix, i, j, visited)
...
def DFS(pixelMatrix, i, j, visited):
    ...
    if (j+1) in range(DIGIT_DATUM_WIDTH):
        next.append((i, j+1))
    # visit all the areas around (i, j)
    for (x, y) in next:
        if visited[x][y] == False and pixelMatrix[x][y] == 0:
            # recursively
            visited = DFS(pixelMatrix, x, y, visited)
    return visited
...
def enhancedPacmanFeatures(state, action):
    ...
    # setup a feature that takes into accounts the closest food to the current pacman lco
    foodLocation = state.getFood().asList()
    closestFoodDist = 0
    if foodLocation:
        # find closest remaining food
        closestFoodDist = min( [util.manhattanDistance(pacmanLocation, food)for food in foodLocation] )
    features['closestFoodDist'] = closestFoodDist
    # setup a feature that tracks the closest ghost to pacman
    ghostLocation = state.getGhostPositions()
    closestGhostDist = 0
    if ghostLocation:
        closestGhostDist = min(util.manhattanDistance(ghost, pacmanLocation)for ghost in ghostLocation)
